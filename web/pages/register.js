...
export default function Register() {
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      const res = await axios.post(`${backend}/api/auth/register`, { email, password, name, apiKey });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color: '#666', marginBottom: '30px', fontSize: '14px' }}>Create your account to get started</p>
        <form onSubmit={handleRegister}>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', color: '#333', fontSize: '14px' }}>Name (optional)</label>
            <input style={{
              width: '100%',
              padding: '12px 16px',
              border: '2px solid #e1e8ed',
          ...
          <button type="submit" style={{
            ...
            transition: 'transform 0.2s'
             ...
             onMouseOut={e => e.target.style.transform = 'translateY(0)'}>Register</button>
          <div style={{ marginTop: 20, textAlign: 'center', fontSize: '14px' }}>
            <a href="/login" style={{ color: '#667eea', textDecoration: 'none', fontWeight: '600' }}>Already have an account? Login</a>
          </div>
        </form>
      </div>
    </div>
